...
FPS::FPS(const char *name, int delayLimitMillis)
    : name(name), delayLimitMillis(delayLimitMillis),
      fpsLimit(ONE_SECOND / delayLimitMillis), startFrameTime(0),
      endFrameTime(0), startLogTime(0), fpsPerSecondTotal(0), minFrameTime(0),
      maxFrameTime(0), allFrameTime(0) {}

void FPS::onStartFrame() { startFrameTime = millis(); }

void FPS::onEndFrame() { endFrameTime = millis(); }
...
void FPS::logEveryOneSecond() {
  ...
  if (!startLogTime) {
    lilka::serial_err("[%s] [%s] The onStartFrame method must be called",
                      LOG_TAG, name);
    return;
  }
  if (!endFrameTime) {
    lilka::serial_err("[%s] [%s] The onEndFrame method must be called", LOG_TAG,
                      name);
    return;
  }
  ...
  if (DEBUG) {
    lilka::serial_log("[%s] [%s] Total: %lu millis, %d fps; Min: %lu millis; "
                      "Avg: %lu; Max: %lu millis",
                      LOG_TAG, name, delta, fpsPerSecondTotal, minFrameTime,
                      allFrameTime / fpsPerSecondTotal, maxFrameTime);
  }
